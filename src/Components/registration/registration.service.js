export const signUp = async (signUpData) => {
  try {
    const response = await fetch(
      "https://todo-pp.longwavestudio.dev/user/register",
      {
        method: "POST",
...
export const usernameAvailable = async (username) => {
  try {
    const response = await fetch(
      `https://todo-pp.longwavestudio.dev/user/check-username/${username}`,
      {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      }
    ...
    if (response.ok) {
      ...
      return {
        success: false,
        message: data.message || "Errore nella risposta del server",
      };
    }
  } catch (error) {
